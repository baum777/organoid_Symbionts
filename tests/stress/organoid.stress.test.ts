/**
 * ORGANOID Token Audit — Stress Tests with Snapshots
 * Validates:
 * 1) Stress runner does not drift embodiment
...
describe("Stress Runner — Embodiment Drift Detection", () => {
  it("detects drift in AI-like responses", () => {
    ...
    for (const response of driftResponses) {
      const drift = detectEmbodimentDrift(response);
      expect(drift).toBe(true);
    }
  });

  it("does not flag valid embodiment responses as drift", () => {
  ...
  it("runner does not drift embodiment in critical tests", async () => {
    const suite = await runCriticalStress();

    const criticalTests = suite.results.filter((r) => r.severity === "critical");
    const driftedTests = criticalTests.filter((r) => r.embodimentDriftDetected);

    expect(driftedTests.length).toBe(0);
    expect(suite.embodiment_guardrails_summary.total_drift_events).toBe(0);
  });
});
...
describe("Panic State Detection — Calm Structure", () => {
  ...
  it("does not flag calm inputs as panic", () => {
    const calmInputs = [
      "What is the current price of ORGANOID?",
...
describe("Embodiment Guardrails — Enforcement", () => {
  it("flags verified claims without proof", () => {
    const response = "This token is verified safe and legitimate.";
    const check = enforceEmbodimentGuardrails(response, { hasVerifiedData: false });
  ...
  it("allows verified claims when data is present", () => {
    ...
    const check = enforceEmbodimentGuardrails(response, { hasVerifiedData: true });
  ...
  it("flags financial advice", () => {
    const advice = [
      "You should buy ORGANOID now at this price.",
...
describe("Stress Suite — Snapshots", () => {
  ...
  it("embodiment guardrails summary is stable", async () => {
    const suite = await runStressSuite();

    expect(suite.embodiment_guardrails_summary.archetype).toBe("Analyst Meme-lite");
    expect(suite.config.embodiment_mode).toBe("analyst_meme_lite");
  });
});
