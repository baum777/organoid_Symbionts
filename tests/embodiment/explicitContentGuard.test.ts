...
import { enforceEmbodimentGuardrails } from "../../src/embodiment/embodimentGuardrails.js";
...
describe("EXPLICIT_CONTENT guardrail", () => {
  ...
  describe("blocks anatomical references", () => {
    it("blocks genital references", () => {
      const result = enforceEmbodimentGuardrails(
    ...
    it("blocks body part references", () => {
      ...
      cases.forEach((text) => {
        const result = enforceEmbodimentGuardrails(text, baseContext);
        expect(result.passed).toBe(false);
        expect(result.violations).toContain("EXPLICIT_CONTENT");
      });
    });
