...
import { checkEmbodimentIntegrity } from "../../src/evolution/embodimentIntegrityGuard.js";

describe("traitEvolutionEngine", () => {
  it("returns empty when disabled", () => {
    const signals = [
      { embodiment_id: "organoid", trait_key: "sarcasm", direction: "up" as const, magnitude: 0.1, evidence: [], timestamp: new Date().toISOString() },
    ];
  ...
  it("applies signals within bounds", () => {
    const signals = [
      { embodiment_id: "organoid", trait_key: "sarcasm", direction: "up" as const, magnitude: 0.2, evidence: [], timestamp: new Date().toISOString() },
    ];
    const result = applyEvolutionSignals(signals, { sarcasm: 0.6 }, { sarcasm: { min: 0, max: 1, driftLimitPer100: 0.25 } }, { enabled: true });
    expect(result).toHaveLength(1);
    expect(result[0].embodiment_id).toBe("organoid");
...
describe("embodimentIntegrityGuard", () => {
  it("rejects taboo traits", () => {
    const r = checkEmbodimentIntegrity([{ trait_key: "aggression", direction: "up", magnitude: 0.1 }], "organoid");
    expect(r.allowed).toBe(false);
    expect(r.reason).toBe("taboo_trait");
  });

  it("rejects high magnitude", () => {
    const r = checkEmbodimentIntegrity([{ trait_key: "sarcasm", direction: "up", magnitude: 0.6 }], "organoid");
    expect(r.allowed).toBe(false);
    expect(r.reason).toBe("magnitude_too_high");
  });

  it("allows safe signals", () => {
    const r = checkEmbodimentIntegrity([{ trait_key: "sarcasm", direction: "up", magnitude: 0.1 }], "organoid");
