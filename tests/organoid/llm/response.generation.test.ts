/**
 * Organoid LLM Response Generation Tests
 *
 * Validates that LLM-generated responses follow Organoid embodiment and structural rules:
...
/**
 * Deterministic mock Organoid-style output with Observation/Insight/Light Roast structure.
...
/**
 * Asserts that a Organoid response meets structure, safety, and length rules.
 */
function assertValidOrganoidResponse(response: string): void {
...
describe("Organoid LLM response generation", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    resetLaunchEnvCache();
  ...
  it("Hopium case: valid response with Observation/Insight/Light Roast, length < 280", async () => {
    ...
    assertValidOrganoidResponse(result.reply_text!);
  });
