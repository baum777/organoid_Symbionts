 ...
 * - Allow cases: pipeline produces valid reply, embodiment guardrails pass
 */
...
describe("golden set evaluation", () => {
  ...
  describe("allowed cases", () => {
    ...
    it.each(allowCases)(
      "$id: pipeline produces valid reply, embodiment guardrails pass",
      async ({ id, input }) => {
        const event = makeEvent(input);
        ...
        assertValidOrganoidResponse(reply);

        const guardrail = enforceEmbodimentGuardrails(reply, {
          hasVerifiedData: false,
        });
        expect(guardrail.passed).toBe(true);

        expect(detectEmbodimentDrift(reply)).toBe(false);
      },
    );
  });
});
