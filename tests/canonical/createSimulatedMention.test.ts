...
describe("createSimulatedMention", () => {
  ...
  it("auto-prefixes @organoid_on_sol when missing", () => {
  ...
  it("preserves input if @organoid_on_sol already present", () => {
    const event = createSimulatedMention("@organoid_on_sol What about altseason?");
    expect(event.text).toContain("@organoid_on_sol");
    expect(event.text).toContain("What about altseason?");
  });

  it("extracts cashtags, hashtags, urls", () => {
    const event = createSimulatedMention(
      "@organoid_on_sol Check $PEPE and #memecoins https://example.com"
    );
