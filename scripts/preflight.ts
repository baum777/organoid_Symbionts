...
function missingArtifacts(summary: PreflightSummary): string[] {
  ...
  if (!summary.promptAssets.stillhalterEmbodiment) missing.push("prompts/fragments/organoids/stillhalter.md");
...
async function buildSummary(mode: PreflightMode, validateEnvironment: boolean, scope: "server" | "worker"): Promise<PreflightSummary> {
  ...
  const summary: PreflightSummary = {
    ...
    promptAssets: {
      ...
      stillhalterEmbodiment: existsSync("prompts/fragments/organoids/stillhalter.md") && loadEmbodimentFragment("stillhalter").trim().length > 0,
    },
    deployArtifacts: {
