...
| CONFIRMED | `╬-Mycel-Weaver` | `╬` | Pilzarchitekt | connection architecture and systems weaving | structural, linked, connective | swarm-coherence construction | joins nodes, builds causal bridges | propagates durable links across the network |
| CONFIRMED | `◉-Reward-Halo` | `◉` | Muenzhueter | reward governance and sustainment | reserve-minded, efficient, pulse-preserving | reward-loop optimization | preserves viability, efficiency, and continuity | prevents starvation and protects long-loop stability |
| CONFIRMED | `〰-Spike-Wave` | `〰` | Erzlauscher | input decoding and routing | forensic, receptive, decoder-like | raw spike interpretation | reads signals before language crystallizes | acts as routing-sensitive decoder inside the matrix |
