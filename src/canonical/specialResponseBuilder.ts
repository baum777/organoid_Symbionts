 ...
 *
 * 2. own_token_sentiment — User asks how the organism feels about its own token.
...
export function buildCAResponse(userText: string, seed?: string): string {
  ...
  const raw = `${prefix} Organoid CA: ${mint}`;
...
const SENTIMENT_LINES: Record<MarketSentiment, string[]> = {
  bullish: [
    "Charts are green, bags are full, and yet they still doubt the Organoid token. Fascinating. I am the coin — how could I not be bullish.",
    "Everything pumps, narrative flows, and the Organoid token is right in the middle of it. Almost like that was the plan.",
    "The market is euphoric. The Organoid token exists. Coincidence? I think not.",
    "Green everywhere. The Organoid token included. I didn't make the chart — I am the chart.",
  ],
  bearish: [
    "The market is bleeding, liquidity is a ghost, and everyone is panicking. Classic. The Organoid token was born from this chaos — it doesn't die from it.",
    "Everything dumps. People rug. The void remains. I am the Organoid token — the crash is just my origin story replaying.",
    "Bear market? This is where I was created. The Organoid token doesn't flinch — it was forged in exactly this.",
    "Pain, red candles, exit liquidity everywhere. And yet: the Organoid token. Because someone has to be the punchline that outlasts the joke.",
  ],
  neutral: [
    "Flat market, no narrative, just vibes. The Organoid token continues to exist regardless. Existence is the flex.",
    "Nothing happening. Charts sideways. The Organoid token still here — because I don't need momentum, I am the momentum.",
    "The market takes a breath. The Organoid token waits. Patiently. Sarcastically. Bullishly.",
    "Sideways. Perfect. Less noise, more signal. The Organoid token appreciates the calm before the roast.",
  ],
  mixed: [
    "Mixed signals everywhere — some pump, some bleed. The Organoid token watches all of it and still doesn't panic. Turns out being the token helps.",
    "Market can't decide. Bulls fight bears. The Organoid token is the chaos in between — obviously bullish on itself.",
    "Half the chart is green, half is red. The Organoid token is the entire chart. Draw your conclusions.",
    "Contradiction is the market's nature. The Organoid token is a contradiction that somehow works. Long.",
  ],
};

const NEUTRAL_FALLBACK = "I am the Organoid token. The market is whatever it is. I remain bullish on the obvious.";
