/**
 * Compose Embodiment Prompt — Build prompt from global + shared + embodiment + event context
 *
 * Used when EMBODIMENTS_ENABLED=true. Composes:
 * - global_rules (safety)
 * - shared_canon
 * - selected_embodiment_profile
...
import { getProfileEmbodiment, getProfileGlyph } from "../embodiments/types.js";
...
export interface EmbodimentRuntimeContext {
  selectedEmbodiment: EmbodimentProfile;
  userProfile?: { familiarity?: number } | null;
  relationshipContext?: unknown;
  sharedLore: string[];
  embodimentMemory: string[];
...
const MODE_STYLE_HINTS: Record<Exclude<CanonicalMode, "ignore">, string> = {
  ...
  social_banter: "Reply casually and in-embodiment.",
  market_banter: "Brief opinionated market take. No financial advice.",
  embodiment_reply: "Answer in-embodiment. Brief, slightly mysterious.",
  lore_drop: "Share lore or backstory. Evocative, compact.",
  conversation_hook: "Short hook that invites further conversation.",
};

/**
 * Compose full LLM prompt from embodiment context.
 */
export function composeEmbodimentPrompt(ctx: EmbodimentRuntimeContext): ComposedEmbodimentPrompt {
  ...
  const profileFragment = loadEmbodimentFragment(ctx.selectedEmbodiment);
  const embodiment = getProfileEmbodiment(ctx.selectedEmbodiment);
  const glyph = getProfileGlyph(ctx.selectedEmbodiment).char;
  ...
  if (profileFragment) {
    parts.push(
      "",
      `Runtime identity: ${ctx.selectedEmbodiment.name} (${ctx.selectedEmbodiment.role}).`,
      `Embodiment anchor: ${embodiment}${glyph ? ` [glyph ${glyph}]` : ""}.`,
      profileFragment,
    );
  }

  if (ctx.selectedEmbodiment.safety_boundaries?.length) {
    parts.push("", "Safety (never override):", ...ctx.selectedEmbodiment.safety_boundaries.map((b) => `- ${b}`));
  }
  ...
  parts.push("", `Max ${charBudget} embodiments.`);
  ...
  if (ctx.embodimentMemory.length > 0) {
    parts.push("", "Recent context:", ...ctx.embodimentMemory.map((m) => `• ${m}`));
  }

  const loreBudget = ctx.responseMode === "hard_caution" || ctx.responseMode === "neutral_clarification" ? 1 : 2;
  const activeLore = getActiveLoreForRole(ctx.selectedEmbodiment.id, loreBudget);
  ...
  const developer = [
    ...
    `Stay under ${charBudget} embodiments.`,
  ].join("\n");
