/**
 * Daily Snippet Extractor — Cron-Job für Embodiment Memory Snippets
 * Liest Audit-Log, filtert nach relevance, extrahiert Roast-Muster via LLM, speichert in Redis.
 *
 * Usage: pnpm exec tsx src/embodiment/dailySnippetExtractor.ts
...
async function runDailySnippetExtraction(): Promise<void> {
  ...
  const userPrompt = `Mentions der letzten Tage (User → Organoid-Antwort):\n\n${mentionsText}\n\nExtrahiere maximal 3 neue Snippets als JSON-Array von Strings.`;
  ...
  try {
    ...
    if (validSnippets.length > 0) {
      ...
      logWarn(
        "[SnippetExtractor] Keine neuen Embodiment-Snippets generiert – möglicherweise zu wenig Daten oder Filter durchgelaufen",
