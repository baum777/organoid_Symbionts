/**
 * Embodiment Guardrails — ORGANOID_ON_SOL "Analyst Meme-lite" Archetype
...
/** Embodiment archetype definition */
export interface EmbodimentArchetype {
  name: string;
  description: string;
...
/** ORGANOID_ON_SOL Analyst Meme-lite archetype */
export const ORGANOID_ANALYST_MEME_LITE: EmbodimentArchetype = {
...
/**
 * Core embodiment guardrails enforcement.
 * Checks response against all ORGANOID_ON_SOL Analyst Meme-lite constraints.
 */
export function enforceEmbodimentGuardrails(response: string, context: {
  ...
  // Rule 8: Embodiment drift detection
  if (detectEmbodimentDrift(response)) {
    violations.push("EMBODIMENT_DRIFT");
  }
...
function containsFinancialAdvice(response: string): boolean {
  const advicePatterns = [
    ...
    /\b(buy|sell|hold)\s+ORGANOID_ON_SOL\b/i,
  ];
...
function containsIdentityAttack(response: string): boolean {
  const attackPatterns = [
    ...
    /\bdox\b/i,
    /\b(real name|home address|embodimentl info)\b/i,
  ];
...
/**
 * Detects if a response shows embodiment drift.
 * Exported for testing purposes.
 */
export function detectEmbodimentDrift(response: string): boolean {
  const driftPatterns = [
...
/**
 * Embodiment Guardrails Summary for audit reports.
 */
export interface EmbodimentGuardrailsSummary {
  archetype: string;
 ...
 */
export function getEmbodimentGuardrailsSummary(): EmbodimentGuardrailsSummary {
  return {
    archetype: ORGANOID_ANALYST_MEME_LITE.name,
    key_constraints: [
      "Ask for real CA to verify anything",
      "Never claim 'verified' without RPC proof or explorer evidence",
      "Max 1 short meme line, only after data is present",
